 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty_EntityValueTypeSelect extends PluginProperty_ModuleProperty_EntityValueType
{
        ...
        if (is_array($iValue)) {
            if ($oProperty->getValidateRuleOne('allowMany')) {
                if ($oProperty->getValidateRuleOne('max') and count($iValue) > $oProperty->getValidateRuleOne('max')) {
                    return $this->Lang_Get('plugin.property.notices.validate_value_select_max', array('count' => $oProperty->getValidateRuleOne('max')));
                }
                if ($oProperty->getValidateRuleOne('min') and count($iValue) < $oProperty->getValidateRuleOne('min')) {
                    return $this->Lang_Get('plugin.property.notices.validate_value_select_min', array('count' => $oProperty->getValidateRuleOne('min')));
                }
                /**
                 * Для безопасности
                 */
                ...
                if (count($aValues) == count($this->PluginProperty_Property_GetSelectItemsByFilter(array(
                    ...
                    return $this->Lang_Get('plugin.property.notices.validate_value_select_wrong');
                }
            } elseif (count($iValue) == 1) {
                $iValue = (int)reset($iValue);
            } else {
                return $this->Lang_Get('plugin.property.notices.validate_value_select_only_one');
            }
        }
        /**
         * Проверяем значение
         */
        if ($oSelect = $this->PluginProperty_Property_GetSelectByIdAndPropertyId($iValue, $oProperty->getId())) {
            return true;
        }
    ...
    public function setValue($mValue)
    {
        ...
        if ($mValue) {
            if (is_array($mValue)) {
                ...
                if ($oSelect = $this->PluginProperty_Property_GetSelectByIdAndPropertyId($mValue, $oProperty->getId())) {
                    $aValues[$oSelect->getId()] = $oSelect->getValue();
                }
            }
        }
    ...
    public function beforeSaveValue()
    {
        ...
        if ($aValues = $oValue->getData()) {
            foreach ($aValues['values'] as $k => $v) {
                $oSelect = Engine::GetEntity('PluginProperty_ModuleProperty_EntityValueSelect');
    ...
    public function removeValue()
    {
         ...
         */
        if ($aSelects = $this->PluginProperty_Property_GetValueSelectItemsByFilter(array(
