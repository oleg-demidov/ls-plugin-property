 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty_EntityProperty extends EntityORM
{
    ...
    protected $aRelations = array(
        'selects' => array(
            ...
            'PluginProperty_ModuleProperty_EntitySelect',
            'property_id',
    ...
    {
        if ($this->PluginProperty_Property_IsAllowPropertyType($this->getType())) {
            return true;
        }
        return $this->Lang_Get('plugin.property.notices.validate_type');
    }

    public function ValidateCheckCode()
    {
        if ($oProperty = $this->PluginProperty_Property_GetPropertyByTargetTypeAndCode($this->getTargetType(), $this->getCode())) {
            if ($this->getId() != $oProperty->getId()) {
                return $this->Lang_Get('plugin.property.notices.validate_code');
            }
        }
        return true;
    }
    ...
    protected function beforeDelete()
    {
        if ($bResult = parent::beforeDelete()) {
             ...
             */
            $this->PluginProperty_Property_RemoveValueByPropertyId($this->getId());
            /**
             * Удаляем значения тегов
             */
            $this->PluginProperty_Property_RemoveValueTagByPropertyId($this->getId());
            /**
             * Удаляем значения селектов
             */
            $this->PluginProperty_Property_RemoveValueSelectByPropertyId($this->getId());
            /**
             * Удаляем сами варианты селектов
             */
            $this->PluginProperty_Property_RemoveSelectByPropertyId($this->getId());
        }
        return $bResult;
    }
