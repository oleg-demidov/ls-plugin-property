 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty_EntityValueTypeImage extends PluginProperty_ModuleProperty_EntityValueTypeFile
{
    ...
    public function validate()
    {
        ...
        if (isset($aValue['tmp_name'])) {
            if (!$aImageInfo = (@getimagesize($aValue['tmp_name']))) {
                return $this->Lang_Get('plugin.property.notices.validate_value_image_wrong');
            }
            /**
             * Проверяем на максимальную ширину
             */
            if ($iWMax = $oProperty->getValidateRuleOne('width_max') and $iWMax < $aImageInfo[0]) {
                return $this->Lang_Get('plugin.property.notices.validate_value_image_width_max', array('size' => $iWMax));
            }
            /**
             * Проверяем на максимальную высоту
             */
            if ($iHMax = $oProperty->getValidateRuleOne('height_max') and $iHMax < $aImageInfo[1]) {
                return $this->Lang_Get('plugin.property.notices.validate_value_image_height_max', array('size' => $iHMax));
            }
        }
