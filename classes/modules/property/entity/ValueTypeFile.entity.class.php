 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty_EntityValueTypeFile extends PluginProperty_ModuleProperty_EntityValueType
{
    ...
    public function validate()
    {
        ...
        if (!$sFileTmpName) {
            if ($oProperty->getValidateRuleOne('allowEmpty')) {
                ...
                return $this->Lang_Get('plugin.property.notices.validate_value_file_empty');
            }
        }
        /**
         * Проверяем на ошибки
        ...
        if ($sFileError and $sFileError != UPLOAD_ERR_NO_FILE) {
            return $this->Lang_Get('plugin.property.notices.validate_value_file_upload') . " - {$sFileError}";
        }
        /**
        ...
        if ($iSizeKb = $oProperty->getValidateRuleOne('size_max') and $iSizeKb * 1024 < $sFileSize) {
            return $this->Lang_Get('plugin.property.notices.validate_value_file_size_max', array('size' => $iSizeKb));
        }
        /**
        ...
        if ($aTypes = $oProperty->getParam('types') and !in_array($aPath['extension'], $aTypes)) {
            return $this->Lang_Get('plugin.property.notices.validate_value_file_type', array('types' => join(', ', $aTypes)));
        }
        /**
