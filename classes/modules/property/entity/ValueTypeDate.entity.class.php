 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty_EntityValueTypeDate extends PluginProperty_ModuleProperty_EntityValueType
{
        ...
        if ($mRes === true) {
            ...
            if ($this->getValueForValidateDate()) {
                ...
                if ($oProperty->getValidateRuleOne('disallowFuture')) {
                    if ($sTimeFull > time()) {
                        return "{$oProperty->getTitle()}: " . $this->Lang_Get('plugin.property.notices.validate_value_date_future');
                    }
                }
                /**
                ...
                if ($oValueObject->_isNew() or strtotime($oValueObject->getValueDate()) != $sTimeFull) {
                    if ($oProperty->getValidateRuleOne('disallowPast')) {
                        if ($sTimeFull < time()) {
                            return "{$oProperty->getTitle()}: " . $this->Lang_Get('plugin.property.notices.validate_value_date_past');
                        }
                    }
                }
            } else {
