 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty_EntityValueTypeImageset extends PluginProperty_ModuleProperty_EntityValueType
{
    ...
    public function validate()
    {
        ...
        if($iMin = $oProperty->getValidateRuleOne('count_min') and $iMin > sizeof($aMediaTargets)){
            return $this->Lang_Get('plugin.property.notices.validate_value_select_min', array('count' => $iMin));
        }

        if($iMax = $oProperty->getValidateRuleOne('count_max') and $iMax < sizeof($aMediaTargets)){
            return $this->Lang_Get('plugin.property.notices.validate_value_select_max', array('count' => $iMax));
        }
