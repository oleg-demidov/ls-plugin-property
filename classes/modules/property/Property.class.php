 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty extends ModuleORM
{
    /**
    ...
    public function Init()
    {
        ...
        if ($aTargetItems = $this->GetTargetItemsByFilter(array('state' => self::TARGET_STATE_ACTIVE))) {
            foreach ($aTargetItems as $oTarget) {
                $this->PluginProperty_Property_AddTargetType($oTarget->getType(), $oTarget->getParams());
            }
        }
    }
    ...
    public function AttachValueForProperties($aProperties, $sTargetType, $iTargetId)
    {
         ...
         */
        $aValues = $this->PluginProperty_Property_GetValueItemsByFilter(array(
            'target_id'      => $iTargetId,
        ...
        foreach ($aProperties as $oProperty) {
            if (isset($aValues[$oProperty->getId()])) {
                ...
                $oProperty->setValue(Engine::GetEntity('PluginProperty_ModuleProperty_EntityValue', array(
    ...
    public function ValidateEntityPropertiesCheck($oTarget)
    {
        ...
        foreach ($aPropertiesObject as $oProperty) {
            ...
            if (true === ($sRes = $oValueType->validate())) {
                ...
                return $this->Lang_Get('plugin.property.notices.validate_value_wrong',
            ...
            if ($aResult) {
                ...
                foreach ($aResult as $aRow) {
                    ...
                    if (!isset($aProperties[$aRow['prop_id']])) {
                        $oProperty = Engine::GetEntity('PluginProperty_ModuleProperty_EntityProperty', $aPropertyData);
                        $aProperties[$aRow['prop_id']] = $oProperty;
                    }
                    if ($aRow['target_id']) {
                        $sKey = $aRow['property_id'] . '_' . $aRow['target_id'];
                        $aValues[$sKey] = Engine::GetEntity('PluginProperty_ModuleProperty_EntityValue', $aValueData);
                    }
                }
                /**
                 * Собираем данные
                ...
                foreach ($aEntitiesWork as $oEntity) {
                    ...
                    foreach ($aProperties as $oProperty) {
                        ...
                        if (isset($aValues[$sKey])) {
                            ...
                            $oValue = Engine::GetEntity('PluginProperty_ModuleProperty_EntityValue', array(
                                'property_type' => $oProperty->getType(),
                                'property_id'   => $oProperty->getId(),
        ...
        if ($aPropFields) {
            ...
            foreach ($aProperties as $oProperty) {
                ...
                if (in_array($oProperty->getType(),
                    array(PluginProperty_ModuleProperty::PROPERTY_TYPE_INT, PluginProperty_ModuleProperty::PROPERTY_TYPE_CHECKBOX))) {
                    $sFieldValue = "value_int";
                    $sConditionFull = $sCondition . ($bIsArray ? ' (?a) ' : ' ?d ');
                } elseif ($oProperty->getType() == PluginProperty_ModuleProperty::PROPERTY_TYPE_FLOAT) {
                    $sFieldValue = "value_float";
                    $sConditionFull = $sCondition . ($bIsArray ? ' (?a) ' : ' ?f ');
                } elseif (in_array($oProperty->getType(), array(
                    PluginProperty_ModuleProperty::PROPERTY_TYPE_VARCHAR,
                    PluginProperty_ModuleProperty::PROPERTY_TYPE_TAGS,
                    PluginProperty_ModuleProperty::PROPERTY_TYPE_VIDEO_LINK
                ))) {
                    $sFieldValue = "value_varchar";
                    $sConditionFull = $sCondition . ($bIsArray ? ' (?a) ' : ' ? ');
                } elseif ($oProperty->getType() == PluginProperty_ModuleProperty::PROPERTY_TYPE_TEXT) {
    ...
    {
        // сбрасываем кеш
        $this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array('PluginProperty_ModuleProperty_EntityValueTag_delete'));
    ...
    {
        // сбрасываем кеш
        $this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array('PluginProperty_ModuleProperty_EntityValueSelect_delete'));
    ...
    {
        $aTags = $this->PluginProperty_Property_GetPropertyTagsGroup($iPropertyId, $iLimit);
        if ($aTags) {
    ...
    public function ChangeTargetType($sType, $sTypeNew)
    {
        ...
        $this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array(
            'PluginProperty_ModuleProperty_EntityProperty_save',
            'PluginProperty_ModuleProperty_EntityTarget_save',
            'PluginProperty_ModuleProperty_EntitySelect_save',
            'PluginProperty_ModuleProperty_EntityValue_save',
            'PluginProperty_ModuleProperty_EntityValueSelect_save',
            'PluginProperty_ModuleProperty_EntityValueTag_save',
        ));
    }
     ...
     * @param bool $bRewrite
     *
     * @return bool|PluginProperty_ModuleProperty_EntityTarget
     */
    public function CreateTargetType($sType, $aParams, $bRewrite = false)
    {
        ...
        if ($oTarget = $this->GetTargetByType($sType)) {
            ...
            $oTarget = Engine::GetEntity('PluginProperty_ModuleProperty_EntityTarget');
    ...
    public function GetPropertiesForUpdate($sTargetType, $iTargetId)
    {
        ...
        $this->PluginProperty_Property_AttachValueForProperties($aProperties, $sTargetType, $iTargetId);
        return $aProperties;
    }
     ...
     *
     * @return bool|PluginProperty_ModuleProperty_EntityProperty
     */
    public function CreateTargetProperty(
        ...
        $oProperty = Engine::GetEntity('PluginProperty_ModuleProperty_EntityProperty');
        ...
        if ($oProperty->_Validate()) {
            if ($oProperty->Add()) {
                ...
                return $this->Lang_Get('plugin.property.notices.create_error');
            }
        } else {
     ...
     *    array(
     *        'data'=>array(
     *        'type'=>PluginProperty_ModuleProperty::PROPERTY_TYPE_INT,
     *        'title'=>'Номер',
     *        'code'=>'number',
     *        'sort'=>100
     *    ),
    ...
    public function CreateDefaultTargetPropertyFromPlugin($aProperties, $sTargetType)
    {
        foreach ($aProperties as $aProperty) {
            ...
            if ($sResultMsg !== true and !is_object($sResultMsg)) {
                 ...
                 */
                $this->RemoveTargetType($sTargetType, PluginProperty_ModuleProperty::TARGET_STATE_NOT_ACTIVE);
                return false;
            }
        }
        return true;
    ...
    public function RemoveValueByPropertyId($iPropertyId)
    {
        ...
        $this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array('PluginProperty_ModuleProperty_EntityValue_delete'));
        return $bRes;
    }
    ...
    public function RemoveSelectByPropertyId($iPropertyId)
    {
        ...
        $this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array('PluginProperty_ModuleProperty_EntitySelect_delete'));
        return $bRes;
    }
