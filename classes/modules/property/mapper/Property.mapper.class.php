 ...
 * @since 2.0
 */
class PluginProperty_ModuleProperty_MapperProperty extends Mapper
{
    ...
    public function GetPropertiesValueByTarget($sTargetType, $iTargetId)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $iTargetId, $sTargetType)) {
            foreach ($aRows as $aRow) {
                ...
                $oProperty = Engine::GetEntity('PluginProperty_ModuleProperty_EntityProperty', $aProperty);
                /**
                ...
                $oProperty->setValue(Engine::GetEntity('PluginProperty_ModuleProperty_EntityValue', $aValue));
    ...
    public function GetPropertyTagsByLike($sTag, $iPropertyId, $iLimit)
    {
        ...
        if ($aRows = $this->oDb->select($sql, $iPropertyId, $sTag . '%', $iLimit)) {
            foreach ($aRows as $aRow) {
                $aReturn[] = Engine::GetEntity('PluginProperty_ModuleProperty_EntityValueTag', $aRow);
            }
        }
        return $aReturn;
    }
    ...
    public function GetPropertyTagsGroup($iPropertyId, $iLimit)
    {
        ...
        if ($aRows = $this->oDb->select(
            ...
            foreach ($aReturn as $aRow) {
                $aReturnSort[] = Engine::GetEntity('PluginProperty_ModuleProperty_EntityValueTag', $aRow);
            }
        }
        return $aReturnSort;
    }
