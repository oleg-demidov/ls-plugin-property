...
class PluginProperty_ActionAdmin_EventProperty extends Event
{
        ...
        if (is_array($aData)) {
            foreach ($aData as $aItem) {
                if (isset($aItem['id']) and isset($aItem['sort'])) {
                    if ($oProperty = $this->PluginProperty_Property_GetPropertyById($aItem['id'])) {
    ...
    {
        $sTargetType = $this->sCurrentEvent;
        if (!$this->PluginProperty_Property_IsAllowTargetType($sTargetType)) {
        ...
        $this->Viewer_Assign('aPropertyTargetParams', $this->PluginProperty_Property_GetTargetTypeParams($sTargetType));
    ...
    public function EventPropertyRemove()
    {
        ...
        if (!($oProperty = $this->PluginProperty_Property_GetPropertyById($this->GetParam(1)))) {
            return $this->EventNotFound();
        }
    ...
    { 
        ...
        if (getRequest('property_update_submit')) {
            ...
            if ($oProperty->_Validate()) {    
                if ($oProperty->Update()) {
                     ...
                     */
                    if ($oProperty->getType() == PluginProperty_ModuleProperty::PROPERTY_TYPE_SELECT) {
                        $aItems = getRequest('items');
                        if (isset($aItems['value']) and is_array($aItems['value'])) {
                            /**
                             * Текущие элементы (для удаления)
                             */
                            $aSelectItems = $this->PluginProperty_Property_GetSelectItemsByFilter(array(
                            ...
                            foreach ($aItems['value'] as $k => $v) {
                                if (isset($aItems['sort'][$k]) and isset($aItems['id'][$k])) {
                                    if ($aItems['id'][$k]) {
                                        if ($oSelect = $this->PluginProperty_Property_GetSelectByFilter(array(
                                                'property_id' => $oProperty->getId(),
                                                'id'          => (int)$aItems['id'][$k]
                                            ))
                                        ) {
                                        ...
                                        $oSelect = Engine::GetEntity('PluginProperty_ModuleProperty_EntitySelect');
                            ...
                            foreach ($aSelectItems as $iId) {
                                if (!in_array($iId,
                                        $aSelectItemsUse) and $oSelect = $this->PluginProperty_Property_GetSelectById($iId)
                                ) {
                                    /**
                                     * Проходимся по всем элементам с этим значением и удаляем его
                                     */
                                    $aValueSelectItems = $this->PluginProperty_Property_GetValueSelectItemsByFilter(array('select_id ' => $oSelect->getId()));
                                    foreach ($aValueSelectItems as $oValueSelect) {
                                        /**
                                         * Формируем свойство для конкретной сущности
                                         */
                                        $aProperties = array($oProperty);
                                        $this->PluginProperty_Property_AttachValueForProperties($aProperties,
                                        ...
                                        if (is_array($aValuesForSelect)) {
                                            ...
                                            $this->PluginProperty_Property_UpdatePropertiesValue($aProperties,
        ...
        $this->Viewer_Assign('layoutBackUrl', Router::GetPath('admin/plugin/property/'.$sTargetType));
        $this->Viewer_Assign('sPropertyTargetType', $sTargetType);
        $this->Viewer_Assign('aPropertyTargetParams', $this->PluginProperty_Property_GetTargetTypeParams($sTargetType));
        $this->SetTemplateAction('update');
    }
